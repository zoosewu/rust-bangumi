...
async fn main() -> anyhow::Result<()> {
    ...
    let mut app = Router::new()
        ...
        .route(
            "/ai-prompt-settings",
            get(handlers::ai_settings::get_ai_prompt_settings)
                .put(handlers::ai_settings::update_ai_prompt_settings),
        )
        .route(
            "/ai-prompt-settings/revert-parser",
            post(handlers::ai_settings::revert_parser_prompt),
        )
        .route(
            "/ai-prompt-settings/revert-filter",
            post(handlers::ai_settings::revert_filter_prompt),
        )
        // 待確認管理
        .route(
            "/pending-ai-results",
            get(handlers::pending_ai_results::list_pending),
        )
        .route(
            "/pending-ai-results/:id",
            get(handlers::pending_ai_results::get_pending)
                .put(handlers::pending_ai_results::update_pending),
        )
        .route(
            "/pending-ai-results/:id/confirm",
            post(handlers::pending_ai_results::confirm_pending),
        )
        .route(
            "/pending-ai-results/:id/reject",
            post(handlers::pending_ai_results::reject_pending),
        )
        .route(
            "/pending-ai-results/:id/regenerate",
            post(handlers::pending_ai_results::regenerate_pending),
        )
        // 健康檢查
